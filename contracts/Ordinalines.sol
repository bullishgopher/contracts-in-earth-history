    ...
    OperatorFiltererUpgradeable
{
    ...
    function initialize(
        ...
        __Ownable_init();
        __PaymentSplitter_init(_paymentSplitterSettings.payees, _paymentSplitterSettings.shares);
        ...
        require(supplyValidationCount == _maxIntendedSupply, "Supply of all phases must equal maxIntendedSupply");
        ...
        _setDefaultRoyalty(_royaltySettings.royaltyAddress, _royaltySettings.royaltyAmount);
     ...
     */
    function mintPhaseAllowlist(uint256 phaseIndex, uint64 quantity, bytes32[] calldata proof) external payable {
    ...
    function mintPhase(uint256 phaseIndex, uint64 quantity) external payable {
        uint64 updatedAmountMinted = _checkPhaseMintConditions(msg.sender, quantity, phaseIndex, msg.value);
    ...
    function mintPublic(uint64 quantity) external payable {
        uint64 updatedAmountMinted = _checkPublicMintConditions(quantity, msg.value);
        _checkGlobalPerWalletMax(msg.sender, quantity);
    ...
    function mintBatch(
        ...
        if (phaseLength > saleState.numPhases || phaseLength != quantities.length || phaseLength != proofs.length) {
            revert InvalidPhase();
        }
        ...
        for (uint256 i = 0; i < phaseLength; ) {
            ...
            if (phase.merkleRoot == bytes32(0)) {
                updatedAmount = _checkPhaseMintConditions(msg.sender, quantity, phaseIndex, priceForPhase);
            } else {
     ...
     */
    function airdropPublic(uint64[] calldata quantities, address[] calldata recipients) external onlyOwner {
        uint256 numRecipients = recipients.length;
        uint256 totalAirdropped;
        if (numRecipients != quantities.length) revert InvalidAirdrop();

        for (uint256 i = 0; i < numRecipients; ) {
            uint64 updatedAmountMinted = baseSettings.amountMinted + quantities[i];
     ...
     */
    function activatePhases(uint256[] calldata phaseIndices, bool activatePublic) external onlyOwner {
     ...
     */
    function pausePhases(uint256[] calldata phaseIndices, bool pausePublic) external onlyOwner {
        uint256 numPhases = phaseIndices.length;
     ...
     */
    function updatePhaseSettings(uint256 phaseIndex, PhaseSettings calldata phase) external onlyOwner {
     ...
     */
    function updateBaseSettings(BaseSettings calldata _baseSettings) external onlyOwner {
    ...
    function _checkAllowlistPhaseMintConditions(
        ...
        if (!MerkleProof.verify(proof, phase.merkleRoot, keccak256(abi.encodePacked(wallet)))) {
            revert InvalidProof();
        }
        ...
        if (phase.maxPerWallet > 0 && updatedAmountMinted > phase.maxPerWallet) {
    ...
    function _checkPublicMintConditions(uint256 quantity, uint256 balance) internal view returns (uint64) {
        if (!isPublicActive) {
            revert PhaseNotActive();
        }
    ...
    function _checkGlobalPerWalletMax(address wallet, uint256 quantity) internal view {
        if (baseSettings.maxPerWallet > 0 && _numberMinted(wallet) + quantity > baseSettings.maxPerWallet) {
            revert ExceedMaxPerWallet();
        }
    }

    function getDataForPhase(uint256 phaseIndex) external view returns (PhaseSettings memory) {
    ...
    function getAmountMintedForPhase(uint256 phaseIndex, address wallet) external view returns (uint64) {
        return amountMintedForPhase[wallet][phaseIndex];
    }

    function getAmountMintedForOwner(address wallet) external view returns (uint256[] memory) {
        uint256[] memory amountMintedPerPhase = new uint256[](saleState.numPhases + 1);
